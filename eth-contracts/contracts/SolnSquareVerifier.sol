...
contract SolnSquareVerifier is ERC721Mintable {
    ...
    function addSolution(uint[2] memory a, uint[2][2] memory b, uint[2] memory c,
        uint[2] memory input, address to, uint256 tokenId) public returns (bool)
    {
        Solution memory Sol = Solution(a, b, c, input,to,tokenId);
    ...
    function mintVerified( bytes32 key, address to, uint256 tokenId) public returns (bool)
    {
        ...
        if(check != true)
        {
            bool verification = verifierContract.verifyTx(sol.a,sol.b,sol.c,sol.input);
...
contract Verifier
{
    function verifyTx(uint[2] memory  a, uint[2][2] memory  b, uint[2] memory  c,
        uint[2] memory  input ) public returns (bool r);
}
